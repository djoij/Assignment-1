...
f_d = pd.read_csv('mergedata_education.csv')
...
f_d=f_d.replace('A & N Islands','Andaman and N Islands')
region=region.replace('A & N Islands','Andaman and N Islands')
f_d.sort_values(['States'],	inplace=True)
f_d=f_d.reset_index()
f_d.drop('index', axis=1, inplace=True)
...
imp_data.to_csv('education_two_columns.csv')
